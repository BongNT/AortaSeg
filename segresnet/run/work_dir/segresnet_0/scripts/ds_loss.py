...
class DeepSupervisionLoss(_Loss):
    ...
    def get_loss(self, input: torch.Tensor, target: torch.Tensor, ske: torch.Tensor) -> torch.Tensor:
        """
        ...
        if input.shape[2:] != target.shape[2:]:
            ...
            ske = F.interpolate(ske, size=input.shape[2:], mode=self.interp_mode)
        return self.loss(input, target, ske)  # type: ignore[no-any-return]

    def forward(self, input: Union[None, torch.Tensor, list[torch.Tensor]], target: torch.Tensor, ske: torch.Tensor) -> torch.Tensor:
        if isinstance(input, (list, tuple)):
            weights = self.get_weights(levels=len(input))
            loss = torch.tensor(0, dtype=torch.float, device=target.device)
            for l in range(len(input)):
                loss += weights[l] * self.get_loss(input[l].float(), target, ske)
            return loss
        if input is None:
            raise ValueError("input shouldn't be None.")

        return self.loss(input.float(), target, ske)  # type: ignore[no-any-return]
